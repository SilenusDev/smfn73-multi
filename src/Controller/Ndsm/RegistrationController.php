...
class RegistrationController extends AbstractController
{
    ...
    #[Route('/register', name: 'ndsm_register')]
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher): Response
    {
        ...
        if ($this->getUser()) {
            return $this->redirectToRoute('ndsm_home');
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Associer le user au site Insidiome
            $site = $this->siteManager->getCurrentSiteEntity();
            $user->setSite($site);

            // Sauvegarder dans la base Insidiome
            $this->dbManager->persist($user);
            $this->dbManager->flush();

            $this->addFlash('success', 'Votre compte a été créé avec succès !');

            return $this->redirectToRoute('ndsm_login');
        }

        return $this->render('insidiome/front/registration/register.html.twig', [
